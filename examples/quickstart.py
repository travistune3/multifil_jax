...
from multifil_jax.simulation import run, get_default_params, SimulationResult
...
start = time.time()
result = run(topo, pCa=4.5, z_line=900.0, duration_ms=100, dt=1.0, verbose=True)
...
result_twitch = run(topo, pCa=pCa_trace, z_line=900.0, duration_ms=200, dt=1.0)
...
result_2d = run(
    topo,
    z_line=[850.0, 900.0, 950.0],  # axis 0
...
# Slice by coordinate
sliced_z = result_2d.sel(z_line=900.0)
print(f"result_2d.sel(z_line=900.0) shape: {sliced_z.axial_force.shape}")  # (3, 2, 100)
...
result_reps = run(topo, pCa=4.5, z_line=900.0, duration_ms=100, replicates=5)
...
result_invert = run(topo_invert, pCa=4.5, z_line=900.0, duration_ms=100)
...
for nrows in [2, 3]:
    ...
    r = run(topo_n, pCa=4.5, z_line=900.0, duration_ms=100)
